...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> _printPdf() async {
    if (_pdfBytes == null) { setState(() { _statusMessage = 'Error: Create PDF first'; }); return; }
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: <Widget>[
          ...
          ElevatedButton(onPressed: _pdfBytes!= null? _printPdf : null, child: const Text('Print PDF')),
          const SizedBox(height: 20),
          if (_pdfBytes!= null)
            Expanded(child: Container(color: Colors.grey[200], child: const Center(child: Text('PDF Created! \nTap Print PDF button above', textAlign: TextAlign.center)))),
        ]),
      ),
    );
  }
